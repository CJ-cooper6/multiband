...
生成视频大小逻辑有错，在5s后计算的不一定是视频的大小



# 缓存设计

hash存储视频sha1值和视频id的映射关系

zset存储视频信息score为id值为视频信息json化的字符串
...
```
ffmpeg -re -i red.mp4 -c copy -preset:v ultrafast -f flv rtmp://127.0.0.1/live1/stream


ffmpeg -re -i input.mp4 -c copy -preset:v ultrafast -f flv rtmp://127.0.0.1/live2/stream
